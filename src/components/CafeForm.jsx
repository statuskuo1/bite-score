...
export function CafeForm({initial,initialDineWith=[],onSave,onSaveAndContinue,onCancel,onMove,onFormChange,weights,addType,setAddType,existingCafes,existingCities,places,onPlaceCreated,user,tasteBudIds,tasteStep=0.1,onTasteStepChange}) {
  const {t} = useLang();
    ...
    <div style={{...S.card,marginBottom:12}}>
      ...
      <div style={{marginBottom:20}}>
        <div style={{display:"flex",gap:10,marginBottom:12}}>
          <div style={S.f1}><FieldLabel>Favourite order</FieldLabel><input type="text" value={f.favOrder||""} onChange={e=>inp("favOrder",e.target.value)} placeholder="e.g. Matcha latte" style={S.wb}/></div>
          <div style={S.f1}><FieldLabel>Shouldn't Get</FieldLabel><input type="text" value={f.shouldntGet||""} onChange={e=>inp("shouldntGet",e.target.value)} placeholder="e.g. Croissant" style={S.wb}/></div>
        </div>
        ...
        <div style={{marginBottom:12}}>
          {!showMoveConfirm ? (
            <button type="button" onClick={()=>setShowMoveConfirm(true)}
              ...
              Logged this as a café? Move to restaurant →
            </button>
          ) : (
            <div style={{background:"#1E1E1C",border:"0.5px solid rgba(255,255,255,0.15)",borderRadius:8,padding:"12px 14px"}}>
              <div style={{fontSize:13,color:"#F1EFE8",fontWeight:500,marginBottom:6}}>Move to restaurant?</div>
              <div style={{fontSize:12,color:"#888780",marginBottom:initialDineWith.length>0?6:10,lineHeight:1.5}}>
                Your scores and notes will carry over. Café-specific fields (drink type, bean notes) won't be kept.
              </div>
