...
export function FaqView({isAdmin, faqOverrides={}, setFaqOverrides}) {
  ...
  const items = [
    ...
    {q:t.faqQ2, a:isZh
      ?<span><span style={{display:"block",marginBottom:6}}>基礎分 = (口味權重 x T) - (CP值權重 x C/P/20) - (等待權重 x W)</span><span style={{display:"block",marginBottom:6}}>W 為對數等待懲罰（見下方）。</span><span style={{display:"block",marginBottom:6}}>最終 BITE = 基礎分 + |基礎分| x R 乘數</span><span style={{display:"block",marginBottom:6}}>畫面上顯示的 0–10 分 = 上述最終 BITE ÷「在你目前權重下理論最佳」（口味 10、無 CP/等待扣減、3 顆星）× 10，再限制在 0–10。</span><span style={{display:"block",marginBottom:4,color:"#888780",fontSize:12}}>此「比例分」的等級區間比直接口味分寬鬆（否則實際用餐幾乎都擠在「好吃」以下）：</span><RatingTierLegend normalized /></span>
      :<span><span style={{display:"block",marginBottom:6}}>Base = (wt x T) - (wb x C/P/20) - (ww x W)</span><span style={{display:"block",marginBottom:6}}>W is the log-scaled wait penalty (see below).</span><span style={{display:"block",marginBottom:6}}>Final raw BITE = Base + |Base| x R multiplier.</span><span style={{display:"block",marginBottom:6}}>The number you see (0–10) is that raw score divided by the best possible raw score for your current weights (taste 10, no buck or wait drag, 3 stars), times 10, clamped to 0–10.</span><span style={{display:"block",marginBottom:4,color:"#888780",fontSize:12}}>Tier labels for that ratio score use wider numeric bands than raw taste (otherwise real visits cluster at “good”). Café lines use the same ratio bands:</span><RatingTierLegend normalized /></span>},
    {q:t.faqQ3, a:isZh
    ...
    {q:t.faqQ4, a:isZh
      ?<span><span style={{display:"block",marginBottom:6}}>口味為 0～10 分。</span><RatingTierLegend /></span>
      :<span><span style={{display:"block",marginBottom:6}}>Taste is rated 0–10.</span><RatingTierLegend /></span>},
    ...
    {q:t.faqQ7, a:isZh
      ?<span>咖啡廳公式為固定加權（約 70% 口味、30% CP 值；$5.25/品項為中性），再除以 1.593。畫面上同樣顯示 0–10：你的分數 ÷ 該公式下理論最佳（口味 10、最佳情況）× 10。「我的品味」→「飲品」的滑桿用於個人化敘述，與此固定公式並行。<br/><br/><span style={bst}><span style={b4}>基礎分 = 0.7 × 口味 − 0.3 × (費用/品項/5.25)</span><span style={b4}>等待懲罰 ≤ |基礎分| 的 10%</span><span style={{display:"block"}}>最終 = (基礎分 − 等待懲罰) + |…| × R 乘數，再 ÷ 1.593</span></span></span>
      :<span>Cafés use a fixed blend in code (~70% taste, 30% bang per buck; $5.25/item neutral), then a divisor of 1.593. The score you see is still 0–10: your raw café score ÷ the theoretical best under that formula × 10, clamped. Sliders under My Taste → Drinks personalize the copy around your priorities alongside this fixed formula.<br/><br/><span style={bst}><span style={b4}>Base = 0.7 × Taste − 0.3 × (Cost / Items / 5.25)</span><span style={b4}>Wait penalty ≤ 10% of |Base|</span><span style={{display:"block"}}>Final raw = (Base − wait penalty) + |…| × R multiplier, then ÷ 1.593</span></span></span>},
  ];
  return (
    <div>
      {items.map(({q,a},i)=>{
