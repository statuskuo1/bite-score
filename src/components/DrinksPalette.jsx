...
export function DrinksPalette({cafes,drinkWeights,replaceDrinkWeights,homeCurrency="USD"}) {
  ...
  const [draftW,setDraftW] = useState(()=>normalizeWeights(drinkWeights));
  const [drinkRoastMode, setDrinkRoastMode] = useState(false);
  useEffect(()=>{if(!editingW)setDraftW(normalizeWeights(drinkWeights));},[drinkWeights,editingW]);
  const livePcts = weightsToPercents(drinkWeights);
    ...
    <div style={{...S.card}}>
      ...
      {editingW?(
        <>
          ...
          <button type="button" onClick={saveDrinkWeights} style={{width:"100%",marginTop:10,padding:"10px",borderRadius:8,border:"none",fontSize:14,fontWeight:500,cursor:"pointer",background:"#F0997B",color:"#141413"}}>{t.weightsSave}</button>
        </>
      ):(
