...
import { calcCafeOutOf10, tasteLabel, CAFE_WEIGHT_DEFAULTS, normalizeWeights, weightsToPercents } from "../utils/scoring.js";
...
export function CafePalette({cafes,cafeWeights,replaceCafeWeights,homeCurrency="USD"}) {
  const {t,lang} = useLang();
  const drinks = cafes; // all cafe visits (Coffee, Tea, Sweets, Other)
  const total = drinks.length;
  const [editingW,setEditingW] = useState(false);
  const [draftW,setDraftW] = useState(()=>normalizeWeights(cafeWeights));
  const [drinkRoastMode, setDrinkRoastMode] = useState(false);
  useEffect(()=>{if(!editingW)setDraftW(normalizeWeights(cafeWeights));},[cafeWeights,editingW]);
  const livePcts = weightsToPercents(cafeWeights);
  function draftUpd(k,v){
    const nv = Math.round(Math.min(10,Math.max(1,+v)));
    setDraftW(w=>({...w,[k]:nv}));
  }
  function saveCafeWeights(){
    replaceCafeWeights(draftW);
    setEditingW(false);
  }
  ...
  const weightsCard = (
    <div style={{...S.card}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        {!editingW
          ? <button type="button" onClick={()=>{setDraftW({...cafeWeights});setEditingW(true);}} style={btnGhost}>{t.editWeights}</button>
      ...
      {editingW?(
        <>
          ...
          <button type="button" onClick={saveCafeWeights} style={{width:"100%",marginTop:10,padding:"10px",borderRadius:8,border:"none",fontSize:14,fontWeight:500,cursor:"pointer",background:"#F0997B",color:"#141413"}}>{t.weightsSave}</button>
        </>
      ):(
  ...
  const scored=drinks.map(e=>({...e,sc:calcCafeOutOf10(e.taste,e.cost,e.portions,e.wait,e.useR,e.repeatability,cafeWeights)})).sort((a,b)=>(b.sc??0)-(a.sc??0));
  const best=scored[0];

  const hasEnoughData = total >= 3;
  const hasMilkData = drinksWithMilk.length >= 3;
  const milkMajority = topMilk && topMilk[1] >= 2 ? topMilk[0] : null;
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{...S.card}}>
        ...
        {(()=>{
          ...
          const scored2=coffeeOnly.map(e=>({...e,sc:calcCafeOutOf10(e.taste,e.cost,e.portions,e.wait,e.useR,e.repeatability,cafeWeights)}));
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
        {[
          ...
          ["Avg BITE", (drinks.reduce((a,e)=>a+(calcCafeOutOf10(e.taste,e.cost,e.portions,e.wait,e.useR,e.repeatability,cafeWeights)??0),0)/total).toFixed(2)],
          [t.avgTaste, avgT+"/10"],
          [t.avgCost, sym+avgC],
          [t.totalCafes, String(total)],
        ].map(([label,val])=>(
